...
class OracleAgent : DatabaseAgent {
    ...
    companion object {
        ...
        /**
         * Rewrite FETCH FIRST / OFFSET-FETCH to ROWNUM for Oracle 11g compatibility.
         *
         * Handles two patterns:
         * 1. SELECT ... FETCH FIRST N ROWS ONLY
         *    → SELECT * FROM (SELECT ...) WHERE ROWNUM <= N
         * 2. SELECT ... OFFSET M ROWS FETCH NEXT N ROWS ONLY
         *    → SELECT * FROM (SELECT a.*, ROWNUM rn__ FROM (SELECT ...) a WHERE ROWNUM <= M+N) WHERE rn__ > M
         */
    ...
    override fun setSchemaSQL(schema: String): String = "ALTER SESSION SET CURRENT_SCHEMA = \"$schema\""
    ...
    override fun executeQuery(sql: String, schema: String?): QueryResult {
        ...
        if (isQuery) {
            // Rewrite FETCH FIRST for Oracle 11g compatibility
            val rewrittenSql = rewriteFetchFirst(trimmedSql)
            conn.createStatement().use { stmt ->
                stmt.executeQuery(rewrittenSql).use { rs ->
